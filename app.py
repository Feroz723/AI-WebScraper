...
def _prepare_pdf_lines(lines, source_url=None, max_chars=95):
    prepared = ["Scraped Text Report"]
    if source_url:
        prepared.append(f"Source: {source_url}")
    prepared.append("")

    if not lines:
        prepared.append("No text content found.")
        return prepared
    ...
    for line in lines:
        ...
        if not normalized:
            continue
        wrapped = textwrap.wrap(normalized, width=max_chars) or [normalized]
...
def _build_text_pdf(lines, source_url=None):
    all_lines = _prepare_pdf_lines(lines, source_url=source_url)
    max_lines_per_page = 48
    pages = []
    for start in range(0, max(1, len(all_lines)), max_lines_per_page):
        pages.append(all_lines[start : start + max_lines_per_page] or ["No text content found."])
    ...
    for page_lines in pages:
        text_rows = ["BT", "/F1 11 Tf", "50 790 Td", "14 TL"]
        ...
        content_id = add_object(f"<< /Length {content_length} >>\nstream\n{stream}\nendstream")
...
def index():
    if request.method == "POST":
        ...
        try:
            ...
            if task == "text":
                pdf_bytes = _build_text_pdf(data, source_url=url)
...
def download_pdf():
    ...
    if not os.path.exists(pdf_path):
        return "No PDF generated yet. Run a text scrape first.", 404
...
def download_image():
    ...
    if not _is_supported_web_url(image_url):
